...
- **분류별 성적**: 그 세트에서 **푼 문제만** 집계해 태그별 정답률을 세트 카드와 같은 카드로 보여줍니다.
  카드는 AWS 영역(스토리지 / 네트워킹 / 컴퓨팅 / 데이터베이스 / 보안 / 애플리케이션 통합 / 분석 / 관리·비용)으로
  나뉘고, 각 그룹 안에서 **약한 분류가 앞으로** 옵니다. 카드를 누르면 그 분류로 푼 문제만 모아 다시 풀 수 있습니다.
  분류를 영역에 매핑하는 표는 `assets/app.js` 의 `TAG_GROUPS` 이며, 새 태그는 자동으로 "기타" 에 들어갑니다.
